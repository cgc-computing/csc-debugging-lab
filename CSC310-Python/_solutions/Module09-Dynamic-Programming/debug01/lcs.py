...
an LCS of the prefixes X[0:i] and Y[0:j], plus a direction table b used
to reconstruct one LCS.  The main block runs the textbook example and a
...
def report(X, Y, show_table):
    ...
    if show_table:
        print_table(c, X, Y)
    print(f"X = {X}, Y = {Y}")
    print(f"  LCS length = {c[m][n]}, one LCS = {build_lcs(b, X, m, n)!r}")
